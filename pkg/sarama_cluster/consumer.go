...
func NewConsumer(addrs []string, groupID string, topics []string, config *Config, osf OffsetStoreFactory) (*Consumer, error) {
	...
	consumer, err := NewConsumerFromClient(client, groupID, topics, osf)
	if err != nil {
		return nil, err
	}
...
func NewConsumerFromClient(client *Client, groupID string, topics []string, osf OffsetStoreFactory) (*Consumer, error) {
	...
	if osf == nil {
		osf = &DefaultOffsetStoreFactory{}
	}
	c.offsetStore = osf.GenOffsetStore(c)
...
func (c *Consumer) CommitOffsets() error {
	...
	req := &offsets2.OffsetCommitRequest{
	...
	resp, err := c.offsetStore.CommitOffset(req)
	if err != nil {
		return err
	}
...
func (c *Consumer) Close() (err error) {
	c.closeOnce.Do(func() {
		...
		close(c.messages)
		close(c.errors)
		log.Info("consumer close message and error channels")
		if e := c.leaveGroup(); e != nil {
			err = e
		}
		close(c.partitions)
		close(c.notifications)
		log.Info("consumer close partition and notification channels")
...
// hbLoop is heartbeat loop, triggered by the mainLoop
...
// twLoop is topic watcher loop, triggered by the mainLoop
...
// cmLoop is commit loop, triggered by the mainLoop
...
// release Releases the consumer and commits offsets, called from rebalance() and Close()
...
// heartbeat Performs a heartbeat, part of the mainLoop()
...
// rebalance Performs a rebalance, part of the mainLoop()
...
// subscribe Performs the subscription, part of the mainLoop()
func (c *Consumer) subscribe(tomb *loopTomb, subs map[string][]int32) error {
	...
	for topic, partitions := range subs {
		for _, partition := range partitions {
			...
			log.Infof("[sarama_cluster] initial offsets: %v", offsets)
...
// joinGroup Send a request to the broker to join group on rebalance()
...
// syncGroup Send a request to the broker to sync the group on rebalance().
...
// fetchOffsets Fetches latest committed offsets for all subscriptions
func (c *Consumer) fetchOffsets(subs map[string][]int32) (map[string]map[int32]offsetInfo, error) {
	...
	req := &offsets2.OffsetFetchRequest{
		ConsumerGroup: c.groupID,
	}
	...
	resp, err := c.offsetStore.FetchOffset(req)
	if err != nil {
		log.Error(err)
		return nil, err
	}
...
// leaveGroup Send a request to the broker to leave the group on failes rebalance() and on Close()
